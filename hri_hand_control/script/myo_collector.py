...
class MyoRaw(object):
    ...
    def __init__(self, n, tty=None):
        ...
        self.acc_tmp = None
        self.gyro_tmp = None
        self.emg_array = deque(maxlen=n)
        self.acc_array = deque(maxlen=n//2)
        self.gyro_array = deque(maxlen=n//2)
        #self.n = n
    ...
    def connect(self):
        ...
        def handle_data(p):
            ...
            if attr in [0x2b, 0x2e, 0x31, 0x34]:
                ...
                ## something
                self.emg_array.append(vals[:8])
                self.emg_array.append(vals[8:16])
                ...
                vals = unpack('10h', pay)
                quat = np.array(vals[:4], float) / 16384.0
                #acc = np.array(vals[4:7], float)  / 2048.0
                #gyro = np.array(vals[7:10], float) / 16.0
                
                self.acc_tmp = [v/2048.0 for v in vals[4:7]]
                self.gyro_tmp = [v/16.0 for v in vals[7:10]]
            else:
